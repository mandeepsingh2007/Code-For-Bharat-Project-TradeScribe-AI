...
def historical_stock_analysis(ticker: str) -> dict:
    ...
    try:
        df = yf.download(
            ...
            interval="1d",
            progress=False,
            auto_adjust=True  # optional: adjust prices
        )
    ...
    # ✅ Fix: convert any non-JSON-compatible keys to str
    latest_data_point = df.iloc[-1][features].round(4).to_dict()
    latest_data_point_clean = {str(k): v for k, v in latest_data_point.items()}
    ...
    return {
        ...
        "latest_data_point": latest_data_point_clean
    }